...
<head>
    ...
    <style>
        :root { --gold: #d4a373; --shiny-gold: #ffefba; --white-glass: rgba(255, 255, 255, 0.95); --accent: #f5e6d3; --red-status: #ff5252; --orange-status: #ff9800; --green-status: #4caf50; }
        body { margin: 0; padding: 0; display: flex; justify-content: center; align-items: center; min-height: 100vh; font-family: sans-serif; background: #faf9f6; overflow: hidden; touch-action: none; }
        
        /* 亮粉特效 */
        .sparkle { position: fixed; width: 4px; height: 4px; background: linear-gradient(45deg, var(--gold), var(--shiny-gold)); border-radius: 50%; pointer-events: none; z-index: 10000; box-shadow: 0 0 5px var(--shiny-gold); animation: glitter 1s forwards ease-out; }
        @keyframes glitter { 0% { transform: scale(0) rotate(0deg); opacity: 1; } 20% { opacity: 1; transform: scale(1.2) rotate(45deg); } 100% { transform: translate(var(--mx), var(--my)) scale(0) rotate(360deg); opacity: 0; } }

        .card { background: var(--white-glass); border-radius: 30px; box-shadow: 0 20px 50px rgba(0,0,0,0.08); width: 92%; max-width: 420px; max-height: 85vh; position: relative; overflow-y: auto; display: flex; flex-direction: column; padding: 30px; box-sizing: border-box; backdrop-filter: blur(15px); z-index: 10; scroll-behavior: smooth; }
        
        /* 絲滑淡入淡出 */
        .step { width: 100%; display: none; opacity: 0; transition: opacity 0.6s cubic-bezier(0.4, 0, 0.2, 1); }
        .visible { display: block; opacity: 1; }
        
        .report-sub-card { background: white; border-radius: 22px; padding: 20px; margin-bottom: 15px; box-shadow: 0 8px 20px rgba(0,0,0,0.03); border: 1px solid rgba(212, 163, 115, 0.1); transition: transform 0.3s ease; text-align: left; }
        .report-sub-card:active { transform: scale(0.98); }
        ...
        /* 鏡頭水平翻轉 (Mirror Effect) */
        video { width: 100%; height: 100%; object-fit: cover; transform: scaleX(-1); }
        #captured-photo, .captured-photo-sync { width: 100%; height: 100%; object-fit: cover; }
        
        .detect-box { position: absolute; border: 2px solid #ff5252; background: rgba(255, 82, 82, 0.1); pointer-events: none; }

        .btn-gold { background: var(--gold); color: white; border: none; padding: 15px; border-radius: 18px; cursor: pointer; width: 100%; font-size: 1rem; font-weight: bold; margin-top: 10px; transition: 0.3s; }
        .btn-detail-link { background: none; border: none; color: var(--gold); font-size: 0.8rem; cursor: pointer; padding: 0; margin-top: 10px; font-weight: bold; text-decoration: underline; }
        
        .tag { display: inline-block; padding: 4px 10px; border-radius: 10px; background: #fdf6ec; font-size: 11px; margin: 2px; color: #b08d57; font-weight: 500; }
        .icon-item { display: flex; align-items: center; margin-bottom: 12px; font-size: 0.9rem; }
        .icon-item span { margin-right: 12px; font-size: 1.4rem; flex-shrink: 0; }
        ...
        #export-container { position: absolute; left: -9999px; width: 600px; height: 600px; background: #fff; padding: 40px; display: grid; grid-template-columns: 1fr 1fr; gap: 30px; }
    </style>
</head>
<body>
...
<div class="card" id="scroll-box">
    <div id="step-0" class="step visible">
        ...
        <div style="text-align:center; margin: 40px 0;"><p id="user-name-display" style="font-size: 1.1rem;">訪客，歡迎回來</p></div>
    ...
    <div id="step-2" class="step">
        ...
        <div style="margin-top:20px;">
            ...
            <select id="user-skin" style="width:100%; padding:14px; border-radius:15px; border:1px solid #eee; margin-top:8px;"><option>混合性</option><option>油性</option><option>乾性</option></select>
            <label style="font-weight:bold; color:var(--gold); font-size:0.85rem; display:block; margin-top:15px;">年齡區間</label>
            <select id="user-age" style="width:100%; padding:14px; border-radius:15px; border:1px solid #eee; margin-top:8px;"><option>20-30</option><option>31-40</option><option>41+</option></select>
        </div>
    ...
    <div id="step-3" class="step">
        ...
        <div class="report-sub-card" style="background: #fffcf9; display: flex; align-items: center; gap: 15px;">
            <div style="position: relative; width: 75px; height: 75px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; background: radial-gradient(closest-side, white 79%, transparent 80% 100%), conic-gradient(#f8d2c9 78%, #eee 0); border-radius: 50%;">
                <div style="text-align: center;"><div style="font-size: 1.3rem; font-weight: bold;">78</div><div style="font-size: 0.45rem; color:#999;">活力指數</div></div>
            </div>
    ...
    <div id="step-b-detail" class="step">
        ...
        <div class="photo-container"><img class="captured-photo-sync"><div class="detect-box" style="top:20%; left:35%; width:30%; height:25%;"></div></div>
        <div class="report-sub-card">
...
<div id="export-container">
    <div style="border-radius: 20px; overflow: hidden;"><img id="export-photo" style="width:100%; height:100%; object-fit:cover;"></div>
    <div style="padding: 30px; display: flex; flex-direction: column; justify-content: center;"><h2 style="color:var(--gold); margin:0; font-weight:300;">SKIN ANALYSIS</h2><canvas id="exportChart" width="250" height="250"></canvas></div>
</div>

<script>
    const video = document.getElementById('video');
    const photoMain = document.getElementById('captured-photo');
    const syncPhotos = document.querySelectorAll('.captured-photo-sync');
    const scrollBox = document.getElementById('scroll-box');
    let userName = "訪客";

    liff.init({ liffId: "2008812290-AidCIwjS" }).then(() => {
        if (liff.isLoggedIn()) liff.getProfile().then(p => { userName = p.displayName; document.getElementById('user-name-display').innerText = userName + "，歡迎回來"; });
    });
    ...
    const createSparkle = (x, y) => {
        ...
        s.style.setProperty('--mx', (Math.random()-0.5)*150+'px'); s.style.setProperty('--my', (Math.random()-0.5)*150+'px');
        document.getElementById('sparkle-container').appendChild(s); setTimeout(() => s.remove(), 1000);
    };
    ...
    function showFinalReport() { switchStep('step-2', 'step-3'); setTimeout(initCharts, 600); }
    ...
    function initCharts() {
        const cfg = { type: 'radar', data: { labels: ['保濕', '泛紅', '毛孔', '暗沉', '痘痘'], datasets: [{ data: [8, 5, 4, 6, 7], backgroundColor: 'rgba(212, 163, 115, 0.1)', borderColor: '#d4a373', borderWidth: 1 }] }, options: { plugins: { legend: { display: false } }, scales: { r: { ticks: { display: false }, min: 0 } } } };
        new Chart(document.getElementById('skinChart'), cfg); new Chart(document.getElementById('exportChart'), cfg);
    }
    ...
    function generateSquareImage() {
        ...
        html2canvas(document.getElementById('export-container'), { scale: 2 }).then(cvs => {
            const link = document.createElement('a'); link.download = 'MySkinReport.png'; link.href = cvs.toDataURL(); link.click();
        });
    }
</script>
</body>
